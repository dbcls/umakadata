...
<<query_for_labels_of_classes_on_a_graph, 9>>.
+
...
<<query_for_datatypes_on_a_graph, 11>>.
+
...
<<query_for_properties_on_a_graph, 13>>.
+
...
<<non_http_uri_subject_on_a_graph, 15>>, and if nothing is found the endpoint
satisfied this rule.
+
...
in Listing <<query_for_a_subject_on_a_graph, 17>> and accessing the URI via HTTP
...
<<query_for_see_also_on_a_graph, 20>>.
+
...
.Listing 24. A Query for Listing Graphs
....
SELECT DISTINCT
  ?g
WHERE {
