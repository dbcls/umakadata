...
1.  Classes
+
If there is GRAPH clause being applied, we retrieve a list of classes using a query described in Listing
...
2.  Labels
+
If there is GRAPH clause being applied, we retrieve a list of labels using a query described in Listing
...
3.  Datatypes
+
If there is GRAPH clause being applied, we retrieve a list of datatypes using a query described in Listing
...
4.  Properties
+
If there is GRAPH clause being applied, we retrieve a list of properties using a query described in Listing
...
. Use HTTP URIs so that people can look up those names
+
We assume all subjects of statements are things. If there is GRAPH clause being applied, we search invalid
statement using a query described in Listing
<<non_http_uri_subject, 10>>, and if nothing is found the endpoint
satisfied this rule.
+
...
.Listing 10. A Query for searching non-HTTP-URI subjects
....
SELECT
  *
WHERE {
...
in Listing <<query_for_a_subject, 11>> and accessing the URI via HTTP
...
+
[[query_for_a_subject]]
.Listing 11. A Query for a Subject
....
SELECT
  ?s
WHERE {
...
using queries described in Listing <<query_for_same_as, 12>>,
<<query_for_see_also, 13>>.
+
[[query_for_same_as]]
.Listing 12. A Query for a Same AS Statement
....
...
+
[[query_for_see_also]]
.Listing 13. A Query for a See Also Statement
....
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
SELECT
  *
...
<<query_ask, 14>>, <<query_for_list_of_graphs, 15>>. The former query is a
...
[[query_ask]]
.Listing 14. A Most Simple Query
....
ASK {}
...
.Listing 15. A Query for Listing Graphs
....
SELECT DISTINCT
  ?g
WHERE {
